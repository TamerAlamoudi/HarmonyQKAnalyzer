...
from config import BASE_DIR, DATA_PATH, DEFAULT_MODELS, FIGURE_PATH
...
def generate_cosine_plots(
    ...
    try:
        output_path = plot_cosine_similarity(
            model_names, data_path, figure_path, weighted=False, logger=logger
        )
    ...
    try:
        output_path = plot_cosine_similarity(
            model_names, data_path, figure_path, weighted=True, logger=logger
        )
        ...
        for model_name in model_names:
            ...
            try:
                output_path = plot_spectrum_and_cosine(
                    model_name, data_path, figure_path, logger=logger
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Plot cosine similarity for transformer models"
    )
    ...
    parser.add_argument(
        "--no_spectrum", action="store_true", help="Do not generate spectrum plots"
    )
    parser.add_argument(
        "--log_dir",
        type=str,
    ...
    generate_cosine_plots(
        model_names, args.data_path, args.figure_path, not args.no_spectrum, log_file
    )
